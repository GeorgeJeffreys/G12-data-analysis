 ...
 *     (The Student-ID → participant join happens in the provider/validation
 *     layer; this parser is roster-agnostic and keys purely on the file's IDs.)
...
export interface ReconciledEssayStudent {
  /** Real Student ID from the file (e.g. `A-A-260506`) — the join key. */
  studentId: string;
...
function resolveColumns(headers: string[]): ColumnMap {
  ...
  return {
    // "student id" (not "essay id", not "student name")
    studentId: findBy((s) => s.includes("student id") || (s.includes("id") && !s.includes("essay") && !s.includes("name")), 2),
 ...
 * Pure — no roster, no provider, no engine. The Student-ID → participant join and
 * the "unknown Student ID" check live in the validation layer.
 */
export function reconcileMasterfile(matrix: string[][], subjectCode: EssaySubjectCode): EssayMasterfileResult {
  ...
  for (const row of matrix.slice(1)) {
    ...
    if (!sr) {
      sr = { studentId, studentName, essays: new Map(), order: studentSeq++ };
 ...
 * student carrying the already-reconciled /20 (`subjectEssay`). Fed through the
 * existing `uploadEssayMarks` path, whose per-student averaging is identity on a
 * single row, so the reconciled value reaches the engine unchanged (halved once).
 * `essayCount` carries the true number of essays for the "pending" disclosure.
 */
export function masterfileToUploadRows(result: EssayMasterfileResult): EssayUploadRow[] {
  return result.reconciled.map((s) => ({
    participantId: s.studentId,
