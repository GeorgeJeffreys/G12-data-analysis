-- 0041 — re-gate every RPC onto app.can_do(action), and remove the bundle layer.
--
-- Recreates each RPC that 0037/0039 gated on app.has_capability(cycle, '<capability>')
-- so it now calls app.can_do(cycle, '<action>') against the granular action grid
-- (migration 0040). Mapping from the R1 capability keys:
--   clean      → clean.rows (row/col removal) · clean.cohort (cohort exclusion)
--   adjust     → grades.adjust (mark change) · incidents.upload (incident rows)
--   configure  → general.config_methodology (element/doc/workspace settings)
--                · general.config_incidents (incident codes / caps / mapping)
--   intake     → upload.manage (clear a sitting)
--   override.marks_exclusions → general.override_marks
--   signoff    → general.signoff
--   workspace_admin → general.delete (sitting/cycle) · general.manage_centres (centres)
-- The member RPCs (invite_member / set_member_role / remove_member → general.manage_users)
-- and the role-admin RPCs moved to 0040 (they also changed shape for role_id).
--
-- awards.generate has NO server RPC — Overall Award documents are generated on the
-- client export path, so that gate is enforced client-side (app/years/.../overall/
-- documents) rather than in a definer function. Flagged here for the record.
--
-- Then removes the R1/R2 bundle layer: app.has_capability, the bundle admin RPCs
-- (create/update/delete_permission, set_role_grant), and the permissions + role_grants
-- tables. (role_permissions was already dropped in 0039.)
--
-- No lib/engine change → scoring parity is untouched (183/183). Run after 0040.

-- ----------------------------------------------------------------------------
-- 1. Re-gate the pipeline + admin RPCs onto app.can_do(action).
-- ----------------------------------------------------------------------------

-- set_clean_removal -> app.can_do(..., 'clean.rows')
...
begin
  if not app.can_do(p_cycle, 'clean.rows') then
...
-- clear_clean_removals -> app.can_do(..., 'clean.rows')
...
-- adjust_participant_mark -> app.can_do(..., 'grades.adjust')
...
declare
  ...
  v_id       uuid;
begin
  if not app.can_do(p_cycle, 'grades.adjust') then
    raise exception 'not authorized';
  end if;
...
-- remove_mark_adjustment -> app.can_do(..., 'grades.adjust')
...
-- clear_incident_rows -> app.can_do(..., 'incidents.upload')
...
-- upsert_incident_code -> app.can_do(..., 'general.config_incidents')
...
declare v_id uuid;
begin
  if not app.can_do(null, 'general.config_incidents') then
    raise exception 'not authorized';
  end if;
...
-- delete_incident_code -> app.can_do(..., 'general.config_incidents')
...
-- set_incident_settings -> app.can_do(..., 'general.config_incidents')
...
-- set_incident_mapping -> app.can_do(..., 'general.config_incidents')
...
-- set_element_labels -> app.can_do(..., 'general.config_methodology')
create or replace function public.set_element_labels(p_config jsonb)
returns void language plpgsql security definer set search_path = public, app as $$
begin
  if not app.can_do(null, 'general.config_methodology') then
    raise exception 'not authorized';
  end if;
...
-- set_document_settings -> app.can_do(..., 'general.config_methodology')
create or replace function public.set_document_settings(p_cycle uuid, p_settings jsonb)
returns void language plpgsql security definer set search_path = public, app as $$
begin
  if not app.can_do(p_cycle, 'general.config_methodology') then
    raise exception 'not authorized';
  end if;
...
-- set_workspace_setting -> app.can_do(..., 'general.config_methodology')
...
-- override_item_exclusion -> app.can_do(..., 'general.override_marks')
...
begin
  ...
  if not app.can_do(v_cycle, 'general.override_marks') then
    raise exception 'not authorized';
  end if;
  if coalesce(btrim(p_reason), '') = '' then
...
-- override_mark_adjustment -> app.can_do(..., 'general.override_marks')
...
begin
  ...
  if not app.can_do(p_cycle, 'general.override_marks') then
    raise exception 'not authorized';
  end if;
  if coalesce(btrim(p_reason), '') = '' then
    raise exception 'an override requires a reason';
...
-- clear_sitting_data -> app.can_do(..., 'upload.manage')
...
begin
  if not app.can_do(p_cycle, 'upload.manage') then
    raise exception 'not authorized';
  end if;
...
-- delete_sitting -> app.can_do(..., 'general.delete')
...
declare
  ...
  v_total bigint;
begin
  if not app.can_do(p_cycle, 'general.delete') then
    raise exception 'not authorized';
  end if;
...
-- delete_cycle -> app.can_do(..., 'general.delete')
...
-- create_test_centre -> app.can_do(..., 'general.manage_centres')
...
begin
  if not app.can_do(null, 'general.manage_centres') then raise exception 'not authorized'; end if;
...
-- update_test_centre -> app.can_do(..., 'general.manage_centres')
...
-- set_test_centre_active -> app.can_do(..., 'general.manage_centres')
...
-- move_exam_year_to_centre -> app.can_do(..., 'general.manage_centres')
...
-- lock_grades -> app.can_do(..., 'general.signoff')
create or replace function public.lock_grades(p_cycle uuid)
returns void language plpgsql security definer set search_path = public, app as $$
begin
  if not app.can_do(p_cycle, 'general.signoff') then
    raise exception 'not authorized';
  end if;
...
-- unlock_grades -> app.can_do(..., 'general.signoff')
...
-- ----------------------------------------------------------------------------
-- 2. Remove the bundle layer (app.has_capability + bundle admin RPCs + tables).
-- ----------------------------------------------------------------------------
drop function if exists public.create_permission(text, text, text[]);
drop function if exists public.update_permission(uuid, text, text, text[]);
drop function if exists public.delete_permission(uuid);
drop function if exists public.set_role_grant(text, uuid, boolean);
drop function if exists app.has_capability(uuid, text);
