-- ============================================================================
-- G12++ — persist the uploaded file size on the import batch.
-- Migration 0009_import_batch_file_size.sql
--
-- Why this exists
--   The Upload screen's file-meta chip showed a stale "exam_export.xlsx · 0 MB"
--   after a refresh: the 3-CSV ingest never persisted the combined upload size,
--   so hydration hard-coded it to 0. (The "exam_export.xlsx" name was a legacy
--   single-file default, removed in the app layer.) This adds a `file_size_mb`
--   column so the real combined size of the three Questionmark CSVs survives a
--   refresh, and re-creates `ingest_persist` to write it.
--
-- No engine/scoring change — display/intake metadata only. Parity unaffected.
--
-- Environment
--   The human runs this in the Supabase SQL editor AFTER 0001–0008. Do not
--   auto-apply. The companion 0009_…rollback.sql reverts it.
-- ============================================================================

begin;

-- ----------------------------------------------------------------------------
-- 1. The combined size (MB) of the uploaded export set. Nullable: legacy rows
--    and any caller that omits it simply have no size (the UI shows none).
-- ----------------------------------------------------------------------------
alter table import_batches add column if not exists file_size_mb numeric;

-- ----------------------------------------------------------------------------
-- 2. Re-create the transactional ingest persist (unchanged from 0007 except the
--    import_batches insert now carries `file_size_mb`). CREATE OR REPLACE keeps
--    the same signature, grants, and SECURITY DEFINER semantics.
-- ----------------------------------------------------------------------------
