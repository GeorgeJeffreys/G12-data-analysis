-- 0039 — configurable permissions: capability catalogue + admin-editable permissions.
--
-- Supersedes the fixed matrix (0036/0037). Inserts an ADMIN-EDITABLE permission
-- layer between roles and the (renamed) CAPABILITIES: capabilities stay code (each
-- a line of enforcement); permissions become data (named bundles of capabilities,
-- created/composed/granted in the UI). Enforcement resolves role → granted
-- permissions → their capabilities → the capability the gate checks.
--
-- No lib/engine change → scoring parity is untouched.
-- Forward-only; run in the Supabase SQL editor after merge.

-- ----------------------------------------------------------------------------
-- 1. permissions — admin-editable bundles of capabilities.
-- ----------------------------------------------------------------------------
create table if not exists public.permissions (
  ...
  is_system    boolean not null default false,   -- the protected Workspace-admin permission
  updated_at   timestamptz default now()
);

-- Seed a default set that reproduces today's access. Idempotent on name.
insert into public.permissions (name, description, capabilities, is_system) values
  ('View',                     'Read-only access to every stage, analytics and reports.', array['view'], false),
  ('Data intake',              'Upload / ingest and manage sittings.',                    array['intake'], false),
  ('Data cleaning',            'Clean data and exclude items/participants.',              array['clean'], false),
  ('Adjustments',              'Adjustments and mark changes.',                           array['adjust'], false),
  ('Cut scores',               'Set grade cut scores.',                                   array['boundaries'], false),
  ('Distinction safeguard',    'Confirm the Distinction caps.',                           array['safeguard'], false),
  ('Sign-off',                 'Lock / sign off a sitting.',                              array['signoff'], false),
  ('Overrides',                'Override another user''s grade-bearing decisions.',       array['override.marks_exclusions','override.distinction'], false),
  ('Audit access',             'Open the Audit log and effective-state surface.',         array['audit.view'], false),
  ('Configuration',            'Configure methodology and incidents.',                    array['configure'], false),
  ('Workspace administration', 'Manage users, permissions, centres and deletion.',        array['workspace_admin'], true)
on conflict (name) do nothing;

-- ----------------------------------------------------------------------------
-- 2. role_grants — which permissions each canonical tier holds.
-- ----------------------------------------------------------------------------
...
-- Seed grants (by permission name) so the three tiers reproduce current access.
insert into public.role_grants (tier, permission_id)
select g.tier, p.id
from public.permissions p
join (values
  ('team_member','View'), ('team_member','Data cleaning'), ('team_member','Adjustments'),
  ('analyst','View'), ('analyst','Data cleaning'), ('analyst','Adjustments'),
  ('analyst','Data intake'), ('analyst','Cut scores'), ('analyst','Distinction safeguard'), ('analyst','Audit access'),
  ('admin','View'), ('admin','Data intake'), ('admin','Data cleaning'), ('admin','Adjustments'),
  ('admin','Cut scores'), ('admin','Distinction safeguard'), ('admin','Sign-off'), ('admin','Overrides'),
  ('admin','Audit access'), ('admin','Configuration'), ('admin','Workspace administration')
) as g(tier, pname) on g.pname = p.name
on conflict (tier, permission_id) do nothing;

-- ----------------------------------------------------------------------------
-- 3. RLS — both tables readable by any signed-in member; no direct client writes.
-- ----------------------------------------------------------------------------
alter table public.permissions enable row level security;
alter table public.role_grants enable row level security;
create policy permissions_select on public.permissions for select using (auth.uid() is not null);
create policy role_grants_select on public.role_grants for select using (auth.uid() is not null);
revoke insert, update, delete on public.permissions from authenticated, anon;
revoke insert, update, delete on public.role_grants  from authenticated, anon;

-- ----------------------------------------------------------------------------
-- 4. app.has_capability — the server gate (replaces app.has_permission). Exists a
--    granted permission for the caller's tier whose capabilities contain p_cap.
--    Workspace-aware exactly like app.has_role.
-- ----------------------------------------------------------------------------
create or replace function app.has_capability(p_cycle uuid, p_cap text)
returns boolean language sql stable security definer set search_path = public, app as $$
  select exists (
    select 1
    from memberships m
    join role_grants rg
      on rg.tier = case m.role
    ...
    where m.user_id = auth.uid()
      ...
      and p_cap = any(p.capabilities)
  );
$$;

-- ----------------------------------------------------------------------------
-- 5. Admin RPCs — create / update / delete permissions + grant/revoke to a tier.
--    All admin-gated (workspace_admin capability) + audited, with the lockout
--    guard: the Workspace-administration system permission can never be deleted,
--    lose the workspace_admin capability, or be un-granted from admin.
-- ----------------------------------------------------------------------------
create or replace function public.create_permission(p_name text, p_description text, p_capabilities text[])
returns uuid language plpgsql security definer set search_path = public, app as $$
declare v_id uuid;
begin
  if not app.has_capability(null, 'workspace_admin') then raise exception 'not authorized'; end if;
  insert into permissions (name, description, capabilities, is_system)
  values (p_name, p_description, coalesce(p_capabilities, '{}'), false)
  returning id into v_id;
  perform app.audit(null, 'create_permission', 'permission', v_id::text, null, to_jsonb(coalesce(p_capabilities, '{}')));
  return v_id;
end $$;

create or replace function public.update_permission(p_id uuid, p_name text, p_description text, p_capabilities text[])
returns void language plpgsql security definer set search_path = public, app as $$
declare v_perm permissions;
begin
  if not app.has_capability(null, 'workspace_admin') then raise exception 'not authorized'; end if;
  select * into v_perm from permissions where id = p_id;
  if not found then raise exception 'no such permission'; end if;
  -- Lockout guard: the Workspace-administration permission must keep workspace_admin.
  if v_perm.is_system and 'workspace_admin' = any(v_perm.capabilities)
     and not ('workspace_admin' = any(coalesce(p_capabilities, '{}'))) then
    raise exception 'cannot remove workspace_admin from the Workspace administration permission';
  end if;
  update permissions
     set name = p_name, description = p_description,
         capabilities = coalesce(p_capabilities, '{}'), updated_at = now()
   where id = p_id;
  perform app.audit(null, 'update_permission', 'permission', p_id::text,
                    to_jsonb(v_perm.capabilities), to_jsonb(coalesce(p_capabilities, '{}')));
end $$;

create or replace function public.delete_permission(p_id uuid)
returns void language plpgsql security definer set search_path = public, app as $$
declare v_perm permissions;
begin
  if not app.has_capability(null, 'workspace_admin') then raise exception 'not authorized'; end if;
  ...
  if v_perm.is_system and 'workspace_admin' = any(v_perm.capabilities) then
    raise exception 'the Workspace administration permission cannot be deleted';
  end if;
  delete from permissions where id = p_id;  -- role_grants cascade
  perform app.audit(null, 'delete_permission', 'permission', p_id::text, to_jsonb(v_perm), null);
end $$;

create or replace function public.set_role_grant(p_tier text, p_permission_id uuid, p_granted boolean)
returns void language plpgsql security definer set search_path = public, app as $$
declare v_perm permissions;
begin
  if not app.has_capability(null, 'workspace_admin') then raise exception 'not authorized'; end if;
  ...
  if not found then raise exception 'no such permission'; end if;
  -- Lockout guard: never un-grant the Workspace-administration permission from admin.
  if p_granted = false and p_tier = 'admin'
     and v_perm.is_system and 'workspace_admin' = any(v_perm.capabilities) then
    raise exception 'the Workspace administration permission cannot be un-granted from admin';
  end if;
  if p_granted then
    insert into role_grants (tier, permission_id) values (p_tier, p_permission_id) on conflict do nothing;
  else
    delete from role_grants where tier = p_tier and permission_id = p_permission_id;
  end if;
  perform app.audit(null, 'set_role_grant', 'role_grant', p_tier || ':' || p_permission_id::text, null, to_jsonb(p_granted));
end $$;

grant execute on function public.create_permission(text, text, text[]) to authenticated;
grant execute on function public.update_permission(uuid, text, text, text[]) to authenticated;
grant execute on function public.delete_permission(uuid) to authenticated;
grant execute on function public.set_role_grant(text, uuid, boolean) to authenticated;

-- ----------------------------------------------------------------------------
-- 6. Re-gate every RPC that 0037 put on app.has_permission onto app.has_capability
--    (same capability key; the two override RPCs → 'override.marks_exclusions').
--    Recreated forward-only from their 0037 definitions; bodies otherwise verbatim.
-- ----------------------------------------------------------------------------
-- set_clean_removal
...
begin
  if not app.has_capability(p_cycle, 'clean') then
...
declare
  ...
  v_id       uuid;
begin
  if not app.has_capability(p_cycle, 'adjust') then
    raise exception 'not authorized';
  end if;
...
declare v_id uuid;
begin
  if not app.has_capability(null, 'configure') then
    raise exception 'not authorized';
  end if;
...
returns void language plpgsql security definer set search_path = public, app as $$
begin
  if not app.has_capability(p_cycle, 'configure') then
    raise exception 'not authorized';
  end if;
...
begin
  ...
  if not app.has_capability(v_cycle, 'override.marks_exclusions') then
    raise exception 'not authorized';
  end if;
  if coalesce(btrim(p_reason), '') = '' then
...
begin
  ...
  if not app.has_capability(p_cycle, 'override.marks_exclusions') then
    raise exception 'not authorized';
  end if;
  if coalesce(btrim(p_reason), '') = '' then
    raise exception 'an override requires a reason';
...
begin
  if not app.has_capability(p_cycle, 'intake') then
    raise exception 'not authorized';
  end if;
...
returns void language plpgsql security definer set search_path = public, app as $$
begin
  if not app.has_capability(p_cycle, 'signoff') then
    raise exception 'not authorized';
  end if;
...
-- ----------------------------------------------------------------------------
-- 7. Retire the fixed matrix (0036/0037) now that nothing references it: the
--    RPCs above were the only callers of app.has_permission.
-- ----------------------------------------------------------------------------
