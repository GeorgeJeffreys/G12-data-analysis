-- 0039 rollback — restore the fixed-matrix model (0036 + 0037) and drop the
-- configurable-permissions layer. Re-runs the 0036 (table + has_permission +
-- set_role_permission + seed) and 0037 (RPCs on has_permission) definitions, then
-- removes the 0039 objects. `create or replace` / `create table if not exists`
-- make the re-applied definitions idempotent.

-- 1. Drop the 0039 admin RPCs, the has_capability gate and the new tables. The RPCs
--    are re-gated back onto has_permission by the embedded 0037 body below, so drop
--    has_capability only after they are recreated (done at the end).
...
-- 2. Recreate the 0036 fixed matrix (role_permissions + app.has_permission +
--    set_role_permission + seed).
...
-- 3. Recreate the 0037 RPC gates on app.has_permission.
...
-- 4. Finally drop the 0039 tables + has_capability (the restored RPCs use
--    app.has_permission again, so nothing references has_capability now).
