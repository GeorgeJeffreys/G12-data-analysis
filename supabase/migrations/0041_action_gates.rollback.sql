-- Rollback 0041 — restore the R1/R2 bundle layer and re-gate every RPC back onto
-- app.has_capability. Run this BEFORE the 0040 rollback. Recreates app.has_capability,
-- the permissions + role_grants tables (empty — re-run 0039's seed to repopulate),
-- the bundle admin RPCs, and the pre-0041 has_capability gates on all 25 RPCs.

-- 1. Recreate the bundle tables (structure only; re-run 0039 to reseed).
create table if not exists public.permissions (
  id uuid primary key default gen_random_uuid(), name text unique not null,
  description text, capabilities text[] not null default '{}',
  is_system boolean not null default false, updated_at timestamptz default now());
...
$$;
-- 3. Recreate the bundle admin RPCs.
...
-- 4. Re-gate the 25 RPCs back onto app.has_capability (verbatim 0039 bodies).
