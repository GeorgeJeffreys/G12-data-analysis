-- ============================================================================
-- Rollback for 0023_ingest_sitting_completeness.sql
--
-- Restores `public.ingest_persist(uuid, jsonb, uuid)` and `public.schema_health()`
-- to their 0022 bodies:
--   * ingest_persist WITHOUT the 0023 sitting-grain completeness guard (the
--     (assessment, participant) roster guard from 0022 is retained).
--   * schema_health reporting migration '0022' (probe surface unchanged).
--
-- No table / column / constraint changes were made by 0023, so there is nothing
-- structural to revert — only the two function bodies. Idempotent.
-- ============================================================================

begin;

-- ingest_persist → 0022 body (no sitting-grain guard).
...
-- schema_health → 0022 body (reports '0022').
