...
function HPipeline({ active = 2, done = 1, compact, range, stages, optIndex }) {
...
function HShell({ active = 'Cycles', crumb, actions, subnav, stage, done, range, stages, optIndex, stageAction, children }) {
  return (
      ...
      <div className="hf-col" style={{ flex: 1, minWidth: 0 }}>
        ...
        {stage != null && (
          <div className="hf-row" style={{ flex: '0 0 auto', borderBottom: `1px solid ${H.line}`, padding: '9px 24px', background: H.canvas, gap: 16, minHeight: 56 }}>
            <HPipeline active={stage} done={done != null ? done : stage} range={range} stages={stages} optIndex={optIndex} />
            <div style={{ flex: 1 }} />
            {stageAction}
          </div>
        )}
