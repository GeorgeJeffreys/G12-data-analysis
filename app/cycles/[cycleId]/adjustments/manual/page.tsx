...
/**
 * Screen — Incident adjustments · Manual override.
 *
 * The retained manual-triage path, kept for incidents the config-driven step
 * doesn't cover (or that a human wants to decide directly): triage each incident
 * into a raw-mark alteration applied to a specific student, a whole subject (bulk),
 * or no action — every alteration a recorded, audit-logged human decision. The
 * DEFAULT surface is the config-driven computation (see "Incident adjustments");
...
export default function ManualAdjustmentsPage({ params }: { params: { cycleId: string } }) {
  ...
  const shellProps = {
    cycleId,
    cycleName,
    page: "Incident adjustments · Manual override",
    stageIndex: 5,
    actions: (
      <Link href={`/cycles/${cycleId}/adjustments`}>
        <Button variant="ghost" title="Back to the config-driven step">Back to incident adjustments</Button>
      </Link>
    ),
    primary: (
      ...
      <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        ...
        <div className="hf-pad" style={{ display: "flex", alignItems: "flex-end", gap: 20, padding: "22px 28px 0", flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 280 }}>
            <div className="hf-h1">Incident adjustments · Manual override</div>
            <div className="hf-sub" style={{ marginTop: 7, maxWidth: 640 }}>
              Manual escape hatch — triage each incident into a raw-mark alteration per student, a whole
              subject, or no action. Use this only for incidents the configured codes don’t cover; the
              default surface is the config-driven computation on the main step.
            </div>
          </div>
