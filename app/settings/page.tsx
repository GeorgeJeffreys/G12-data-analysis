...
/**
 * Settings → Grading defaults. Configure the grade vocabulary and default
 * cut-points used across the suite: the per-assessment performance levels +
 * star mapping, the overall award levels, and the default cut-points for each.
 * Edits are committed to the provider (in memory in this build).
 */
...
export default function SettingsPage() {
  ...
  // Local editable form, seeded from the provider. Stars are edited per level
  // index (so renaming a level can't desync the star map).
  ...
  return (
    <Shell crumb={[{ label: "Settings" }, { label: "Grading defaults" }]}>
      <div style={{ display: "flex", flexDirection: "column", padding: "28px 32px", gap: 22, flex: 1, maxWidth: 980 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <div className="hf-h1">Grading defaults</div>
            <div className="hf-sub" style={{ marginTop: 7 }}>
              The named levels, star mapping and default cut-points used when a new cycle’s boundaries are set. Cut-points are per-cycle adjustable on the boundaries screen.
            </div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <Button variant="pri" disabled={!dirty} onClick={save}>Save defaults</Button>
          </div>
        </div>
            ...
            <tbody>
              {perfLevels.map((lvl, i) => (
                <tr key={i}>
                  ...
                  <td className="hf-td">
                    <TextField value={lvl} onChange={(v) => setPerfLevels(replaceAt(perfLevels, i, v))} />
                  </td>
                  <td className="hf-td">
                    <TextField value={stars[i] ?? ""} width={90} mono onChange={(v) => setStars(replaceAt(stars, i, v))} placeholder="(blank)" />
                  </td>
            ...
            <tbody>
              {awardLevels.map((lvl, i) => (
                <tr key={i}>
                  <td className="hf-td hf-mono" style={{ color: H.ink3, width: 30 }}>{i + 1}</td>
                  <td className="hf-td">
                    <TextField value={lvl} onChange={(v) => setAwardLevels(replaceAt(awardLevels, i, v))} />
                  </td>
                  <td className="hf-td" style={{ textAlign: "right" }}>
